...
export function MusicPlayer({ musicUrl, isPlaying: initialPlayState }: MusicPlayerProps) {
  ...
  return (
    <div className="fixed bottom-6 left-6 z-40 flex items-center gap-2">
      ...
      <button
        onClick={togglePlay}
        className={`w-12 h-12 bg-white/80 backdrop-blur rounded-full shadow-lg flex items-center justify-center text-gray-800 transition-transform ${isPlaying ? 'animate-spin-slow' : ''}`}
      ...
      <button
        onClick={toggleMute}
        className="w-10 h-10 bg-white/80 backdrop-blur rounded-full shadow-lg flex items-center justify-center text-gray-600"
      >
        {isMuted ? (
