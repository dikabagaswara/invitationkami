  ...
  return (
    <div className="font-serif bg-amber-50 text-stone-800 min-h-screen">
      ...
      {sectionConfig.quote && invitation.quote && (
        <section className="py-20 px-8 text-center max-w-2xl mx-auto">
          ...
          {invitation.quoteSource && (
            <p className="mt-4 font-semibold text-sm uppercase tracking-wider text-amber-700">
              - {invitation.quoteSource} -
            </p>
          )}
        </section>
        ...
        <section className="py-24 px-8 max-w-4xl mx-auto text-center relative">
          ...
          <div className="grid md:grid-cols-2 gap-16">
            <div className="space-y-6">
              <div className="w-56 h-72 mx-auto bg-amber-100 rounded-t-full shadow-lg overflow-hidden border-8 border-white">
                {invitation.groomPhoto ? (
                  <img src={invitation.groomPhoto} alt="Groom" className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full bg-amber-200/50"></div>
                )}
              </div>
              <div>
      ...
      {sectionConfig.countdown && events.length > 0 && (
        <section className="py-20 px-8 bg-amber-100/50 text-center border-y border-amber-200">
          <h2 className="text-3xl italic text-amber-700 mb-12 font-light">Save the Date</h2>
      ...
      {sectionConfig.events && events.length > 0 && (
        <section className="py-24 px-8 bg-white text-center">
          ...
          <div className="max-w-3xl mx-auto space-y-12">
            {events.map(event => (
              <div key={event.id} className="p-10 border border-amber-200 shadow-sm relative bg-amber-50/30">
                <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white px-4 text-amber-500 text-2xl">❧</div>
                <h3 className="text-3xl mb-4 font-semibold">{event.title}</h3>
                <p className="mb-2 font-semibold text-lg text-amber-800">
                ...
                {event.mapUrl && (
                  <a href={event.mapUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-4 px-6 py-2 border border-amber-600 text-amber-700 hover:bg-amber-600 hover:text-white transition-colors rounded-full text-sm uppercase tracking-widest">
                    Buka Google Maps
                  </a>
                )}
              </div>
        ...
        <section className="py-24 px-8 max-w-4xl mx-auto text-center">
          ...
          <div className="space-y-12 relative before:absolute before:inset-0 before:ml-1/2 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-amber-200">
            {loveStory.map((story, i) => (
              <div key={story.id} className="relative z-10 bg-amber-50 p-6 md:p-8 border border-amber-200 shadow-sm max-w-lg mx-auto">
                <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-4 h-4 rounded-full bg-amber-500 border-4 border-amber-50"></div>
                <h3 className="text-2xl mb-2 font-semibold text-amber-800">{story.title}</h3>
                {story.date && <p className="text-sm font-semibold text-amber-600 mb-4">{story.date}</p>}
      ...
      {sectionConfig.gallery && gallery.length > 0 && (
        <section className="py-24 px-8 bg-amber-100/50">
          ...
          <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
            {gallery.map(item => (
              <div key={item.id} className="aspect-square bg-amber-200 overflow-hidden shadow-md border-4 border-white">
