...
export default async function GuestsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function addGuest(formData: FormData) {
    ...
    revalidatePath(`/invitations/${id}/guests`)
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">Daftar Tamu & Link Spesial</h1>
        <p className="text-muted-foreground mt-1">
          Kelola daftar tamu Anda dan bagikan link undangan khusus yang menyertakan nama mereka.
        </p>
      </div>
        ...
        <CardContent>
          {guests.length === 0 ? (
            <p className="text-center py-8 text-muted-foreground text-sm">
              Belum ada tamu yang didaftarkan.
            </p>
          ) : (
