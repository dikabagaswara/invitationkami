...
export function ShareGeneratorClient({
  ...
  const selectedInvitation = invitations.find((inv) => inv.id === selectedInvId) || invitations[0]

  const coupleTitle = selectedInvitation
    ? `${selectedInvitation.groomName} & ${selectedInvitation.brideName}`
    : 'Mempelai'
  ...
  const generatedShares = guestList.map((name, index) => {
    const slug = selectedInvitation?.slug || 'undangan'
    const link = `${baseUrl}/i/${slug}?to=${encodeURIComponent(name)}`
      ...
      <div className="bg-gradient-to-r from-stone-900 via-stone-800 to-stone-900 text-white rounded-3xl p-6 sm:p-10 shadow-lg relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-3">
          ...
          <p className="text-stone-300 text-xs sm:text-sm font-light max-w-2xl leading-relaxed">
            Ketik daftar nama tamu (satu baris per nama seperti <em>&quot;Dika dan Istri&quot;</em>, <em>&quot;Nurdi dan Istri&quot;</em>). Sistem akan otomatis mengonversi nama menjadi link undangan khusus dan draf pesan WhatsApp siap kirim.
          </p>
        </div>
      </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          <Card className="rounded-2xl border-stone-200 shadow-xs">
            <CardHeader>
              ...
              <CardDescription>
                Pilih undangan yang ingin Anda buatkan tautan tamunya.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="invitation-select">Pilih Acara Undangan</Label>
